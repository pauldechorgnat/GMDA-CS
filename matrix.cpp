...
QSMatrix<T>::QSMatrix(unsigned _rows, unsigned _cols, const T& _initial) {
    if (_initial == 'r') { // generate a random matrix

        srand(time(NULL)+_rows*100+_cols*100);
        double number = 0;
    ...
    else if (_initial == 'i') { // generate identity matrix
        mat.resize(_rows);
        for (unsigned i=0; i<mat.size(); i++) {
            mat[i].resize(_cols, 0);
            mat[i][i] = 1.0;
...
// Print a matrix
template<typename T>
QSMatrix<T> QSMatrix<T>::print_mat() {

    QSMatrix result(1, 1, 0.0);
