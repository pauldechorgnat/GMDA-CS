...
struct Node {
    ...
    int node_depth;
    Node(DataSet dataset, DataSet base, int index_direction, int depth, int minCellSize, long long int id);
};
...
// building the tree 
Node::Node( DataSet dataset_input, DataSet base, int index_direction, int depth = 0, int minCellSize = 10, long long int id = 0) {
	node_id =id;
    ...
    if ( dataset_left.size()>0){
	    left  = new Node(dataset_left , base, (index_direction + 1) % dimension, depth + 1, minCellSize, 2* id );
    }
    if ( dataset_right.size()>0){
	    right = new Node(dataset_right,base, (index_direction + 1) % dimension, depth + 1, minCellSize, 2* id + 1);
    }
}
...
int main(){
	...
	int simulation_number_of_points = 2000; 
	...
	string output_path = github_repo_path + "results_auslan/";
	string input_path = github_repo_path + "input_data/auslan";
